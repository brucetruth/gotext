...
func (g *GoTextError) Error() string {
	...
	if os.Getenv("GOLEARN_FULL_DEBUG") == "true" {
		return fmt.Sprintf("GoTextError( %s\n\tCaptured at: %s\n)",
	...
	return fmt.Sprintf("GoTextError( %s: %s )", g.Description, g.WrappedError)
}

func (g *GoTextError) attachFormattedStack() {
	...
	for i := 0; i < len(stackFrames); i++ {
		if strings.Contains(stackFrames[i], "golearn") {
			...
			if strings.Contains(stackFrames[i], "golearn/base.(*GoTextError).attachFormattedStack") {
				continue
			}
